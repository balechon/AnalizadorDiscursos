...
class ResumeOutputParser(BaseModel):
    summary: str = Field(description="Resumen del discurso")

class IdeaOutputParser(BaseModel):
    ideas: list[str] = Field(description="Lista de ideas extraídas del texto")
...
class GPT:
    ...
    def create_resume(self, text: str) -> dict:
        """
        Crea un resumen del texto proporcionado.

        :param text: Texto a resumir
        :return: Diccionario con el resumen
        """
        try:
            parser = JsonOutputParser(pydantic_object=ResumeOutputParser)
            prompt = PromptTemplate(
                template="Resume el siguiente discurso en español, capturando los puntos principales y el mensaje central. El resumen debe ser conciso pero informativo, incluyendo las ideas clave y cualquier argumento importante presentado. Evita detalles innecesarios y céntrate en la esencia del discurso. El resumen debe tener aproximadamente 1-2 párrafos. Trata de que tenga una Introducción, Desarrollo y Conclusión\n{format_instructions}\nDiscurso:\n{text}\n",
    ...
    def divide_into_ideas(self, text: str) -> list[str]:
        """
        Divide el texto de entrada en sus ideas componentes.

        :param text: Texto a dividir en ideas
        :return: Lista de ideas extraídas directamente del texto
        """
        try:
            parser = JsonOutputParser(pydantic_object=IdeaOutputParser)
            prompt = PromptTemplate(
                template="""
Divide the text into numbered sections, each representing a complete and substantial idea or theme. Follow these rules:

1. Each section MUST contain multiple sentences (at least 3-5) that together form a coherent thought, argument, or theme.
2. Closely related ideas should be grouped together in a single section, even if this results in longer sections.
3. Do NOT alter, summarize, or paraphrase the text in any way. Use the exact original wording.
4. Include every word from the original text.
5. Maintain the original order of the text.
6. Aim for sections that are roughly 5-10 sentences long, but prioritize keeping related ideas together over maintaining a specific length.
...
{format_instructions}
            ...
            logger.error(f"Error dividing text into ideas: {str(e)}")
            raise
...
#     try:
#         ideas = gpt.divide_into_ideas(sample_text)
#         print("Ideas extraídas del texto:")
...
#         df_sentiment = pd.DataFrame(list_sentimen_analisis)
#         print(df_sentiment.head())
#     except Exception as e:
#         logger.error(f"Ocurrió un error al dividir el texto en ideas: {str(e)}")
